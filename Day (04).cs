...
foreach (var line in input.Split(Environment.NewLine))
{
    ...
    var name = split.SkipLast(2).SelectMany(x => x).ToList();
    var sector = int.Parse(split[^2]);
    var checksum = split.Last().ToList();
...
{
    var grpd = chars.GroupBy(x => x).OrderByDescending(x => x.Count()).ThenBy(x => x.First()).ToList();
    return checksum.Select((x, i) => (x, i)).All(x => grpd[x.i].Key == x.x);
}
