...
//foreach (var item in input.Split(Environment.NewLine).Where(x => x.StartsWith("value")).ToList())
//{
//    var split = item.Split(new[] { "value ", " goes to bot " }, StringSplitOptions.RemoveEmptyEntries).Select(int.Parse);
//    var value = split.First();
//    var bot = split.Last();
//    if (!bots.ContainsKey(bot))
//    {
//        bots[bot] = new List<int>();
//    }
//    bots[bot].Add(value);
//}

//var toVisit = input.Split(Environment.NewLine).Where(x => x.StartsWith("bot")).ToHashSet();
//while (toVisit.Count > 0)
//{
//    foreach (var item in toVisit.ToList())
//    {
//        var split = item.Replace("output ", "bot -11").Split(new[] { "bot ", " gives low to bot ", " and high to bot " }, StringSplitOptions.RemoveEmptyEntries).Select(int.Parse);
//        var bot = split.First();
//        if (!bots.ContainsKey(bot) || bots[bot].Count != 2) { continue; }
//        var low = split.ElementAt(1);
//        var high = split.ElementAt(2);
//        if (!bots.ContainsKey(low))
//        {
//            bots[low] = new List<int>();
//        }
//        if (!bots.ContainsKey(high))
//        {
//            bots[high] = new List<int>();
//        }
//        bots[low].Add(bots[bot].Min(x => x));
//        bots[high].Add(bots[bot].Max(x => x));
//        toVisit.Remove(item);
//    }
//}


//result = new[] { -110, -111, -112 }.Aggregate(1, (x, y) => x * bots[y].Single());
