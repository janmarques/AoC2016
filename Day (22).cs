...
var nodes = input.Split(Environment.NewLine).Skip(2).Select(line =>
{
    var split = line.Replace("T", "").Split(" ", StringSplitOptions.RemoveEmptyEntries);
    var size = int.Parse(split[1]);
    var used = int.Parse(split[2]);
    var avail = int.Parse(split[3]);

    var nameSplit = split[0].Replace("x", "").Replace("y", "").Split("-");
